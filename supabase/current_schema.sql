...
create table if not exists public.destination_services (
  ...
  pricing_basis text not null default 'per_day' check (pricing_basis in ('per_day', 'per_night')),
...
create table if not exists public.booking_guest_visits (
  ...
  constraint booking_guest_visits_checkout_requires_checkin
    check (checked_out_at is null or checked_in_at is not null),
  constraint booking_guest_visits_time_order
...
alter table public.destination_services
  ...
  add column if not exists open_weekdays smallint[] not null default array[1, 2, 3, 4, 5, 6, 7]::smallint[],
  add column if not exists operating_remarks text;

update public.destination_services
set image_paths = case when image_path is null then '{}'::text[] else array[image_path] end,
    image_urls = case when image_url is null then '{}'::text[] else array[image_url] end
where cardinality(image_paths) = 0 and (image_path is not null or image_url is not null);

alter table public.destination_services
  drop constraint if exists destination_services_operating_times_check,
  drop constraint if exists destination_services_open_weekdays_check,
  drop constraint if exists destination_services_service_photos_check;

alter table public.destination_services
  add constraint destination_services_operating_times_check
    check (opening_time is null or closing_time is null or opening_time < closing_time),
  add constraint destination_services_open_weekdays_check
    check (cardinality(open_weekdays) between 1 and 7 and open_weekdays <@ array[1, 2, 3, 4, 5, 6, 7]::smallint[]),
  add constraint destination_services_service_photos_check
    check (cardinality(image_paths) <= 5 and cardinality(image_urls) <= 5 and cardinality(image_paths) = cardinality(image_urls));
...
create index if not exists booking_guest_visits_booking_idx
  on public.booking_guest_visits (booking_id, guest_number);
...
create trigger booking_guest_visits_touch_updated_at
before update on public.booking_guest_visits
for each row execute function public.touch_updated_at();
