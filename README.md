...
PAYMONGO_PAYMENT_METHODS=gcash,card,grab_pay,paymaya,billease
BREVO_API_KEY=
BOOKING_RECEIPT_FROM_EMAIL=
...
- `PAYMONGO_PAYMENT_METHODS` should contain only the methods activated for your PayMongo merchant.
- `BREVO_API_KEY` and `BOOKING_RECEIPT_FROM_EMAIL` enable the detailed post-payment booking receipt. The sender address must be verified in Brevo.
- `BOOKING_SUPPORT_EMAIL` is shown in official receipt emails for traveler inquiries.
...
- partial refunds or refunds after check-in/payout settlement
