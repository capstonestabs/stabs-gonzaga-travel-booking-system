...
export type CheckoutPaymentMethod = (typeof supportedCheckoutPaymentMethods)[number];

export function getCheckoutPaymentMethods(): CheckoutPaymentMethod[] {
  const configured = env.paymongoPaymentMethods
    .split(",")
    .map((method) => method.trim().toLowerCase())
    .filter((method): method is CheckoutPaymentMethod =>
      supportedCheckoutPaymentMethods.includes(method as CheckoutPaymentMethod)
    );

  return configured.length ? Array.from(new Set(configured)) : ["gcash"];
}
  ...
  const payload = {
    data: {
      attributes: {
        ...
        payment_method_types: getCheckoutPaymentMethods(),
        line_items: items,
        metadata: {
