...
import { getIsoWeekday, normalizeOpenWeekdays } from "@/lib/service-schedule";
...
export async function getServiceAvailabilityMonthStatuses(
  ...
  const [
    ...
    supabase
      ...
      .select("daily_capacity, is_active, availability_start_date, availability_end_date, open_weekdays")
      .eq("id", serviceId)
      .maybeSingle(),
    supabase
  ...
  const openWeekdays = normalizeOpenWeekdays(service?.open_weekdays as number[] | null);
  ...
  return monthDates.map((date) => {
    ...
    const isRecurringOpenDay = openWeekdays.includes(getIsoWeekday(date));

    if (!isServiceActive || dailyCapacity <= 0 || closedDates.has(date) || outsideWindow || !isRecurringOpenDay) {
      return {
        date,
