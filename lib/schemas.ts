  ...
  .superRefine((value, context) => {
    if (value.guestTypes && value.guestTypes.length !== value.guestCount) {
      context.addIssue({
        ...
        path: ["guestTypes"],
        message: "Guest details must match the total number of guests."
      });
    }
    if (value.guestDetails && value.guestDetails.length !== value.guestCount) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["guestDetails"],
        message: "Guest names must match the total number of guests."
      });
    }
  });
...
export const checkoutDraftSchema = z.object({
  ...
  notes: z.string(),
  policies: z.array(z.string()).max(100).optional(),
  additionalServices: z
    .array(
      z.object({
        id: z.string().uuid(),
