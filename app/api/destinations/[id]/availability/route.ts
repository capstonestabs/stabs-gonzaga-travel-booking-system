...
export async function GET(
  ...
  try {
    ...
    const { data: service, error: serviceError } = await supabase
      ...
      .select("id, daily_capacity, is_active, availability_start_date, availability_end_date, open_weekdays")
      .eq("id", serviceId)
    ...
    if (
      ...
      ((service.availability_start_date && serviceDate < service.availability_start_date) ||
        ...
        !normalizeOpenWeekdays(service.open_weekdays as number[] | null).includes(getIsoWeekday(serviceDate)))
    ) {
      return NextResponse.json({
        availability: {
          is_open: false,
