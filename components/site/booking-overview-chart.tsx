...
export function BookingOverviewChart({ data }: { data: BookingActivityPoint[] }) {
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden dashboard-glass-panel">
      <div className="flex w-full items-center justify-between gap-3 px-4 pb-1 pt-3.5">
        <h3 className="m-0 font-display font-semibold">Booking Overview</h3>

        <div className="flex shrink-0 items-center gap-1 rounded-[0.7rem] border border-white/60 bg-white/40 p-1">
          <Button
            type="button"
            variant="ghost"
            size="sm"
            className={`h-7 min-h-7 rounded-[0.55rem] px-2.5 text-xs font-medium ${
              granularity === "daily" ? "bg-white/70 text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
            }`}
          ...
          <Button
            ...
            size="sm"
            className={`h-7 min-h-7 rounded-[0.55rem] px-2.5 text-xs font-medium ${
              granularity === "weekly" ? "bg-white/70 text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
            }`}
      ...
      <div className="flex-1 space-y-1.5 p-4 pt-1">
