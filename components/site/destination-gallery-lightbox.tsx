...
export function DestinationGalleryLightbox({
  ...
  const heroImages = safeImages.slice(0, 5);
    ...
    <>
      {layout === "hero" ? (
        <section
          ...
          className={cn(
            "relative grid min-h-[17rem] gap-1.5 overflow-hidden rounded-[1.35rem] bg-muted shadow-[0_18px_42px_rgba(22,74,47,0.12)] sm:min-h-[25rem] sm:gap-2",
            heroImages.length >= 3 && "grid-cols-2 sm:grid-cols-4 sm:grid-rows-2",
        ...
        >
          {heroImages.map((image, index) => (
            <button
              ...
              className={cn(
                "group relative min-h-[8rem] overflow-hidden bg-muted text-left",
                heroImages.length >= 3 && index === 0 && "col-span-2 row-span-2 min-h-[17rem] sm:min-h-[25rem]",
                heroImages.length >= 3 && index > 2 && "hidden sm:block"
              )}
          ...
          <button
            ...
            className="absolute bottom-3 right-3 inline-flex min-h-10 items-center gap-2 rounded-full border border-white/70 bg-white/94 px-4 text-xs font-semibold text-foreground shadow-lg backdrop-blur transition hover:-translate-y-0.5 hover:bg-white sm:bottom-4 sm:right-4 sm:text-sm"
          >
            <Expand className="h-4 w-4" />
            Show all {safeImages.length} photo{safeImages.length === 1 ? "" : "s"}
          </button>
        </section>
      ) : (
