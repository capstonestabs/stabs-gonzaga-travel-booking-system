    ...
    <Card className="overflow-hidden">
      ...
      <CardContent className="space-y-4">
        ...
        {booking.payment?.refund_status ? (
          <p className="rounded-[0.95rem] border border-sky-200 bg-sky-50 px-3.5 py-3 text-sm text-sky-900">
            Refund status: <strong>{booking.payment.refund_status.replaceAll("_", " ")}</strong>
            {booking.payment.refund_amount ? ` · ${formatCurrency(booking.payment.refund_amount)}` : ""}
            {booking.payment.paymongo_refund_id ? ` · Reference ${booking.payment.paymongo_refund_id}` : ""}
          </p>
        ) : null}
