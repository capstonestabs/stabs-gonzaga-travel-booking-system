...
import { formatOpenWeekdays, getIsoWeekday, normalizeOpenWeekdays } from "@/lib/service-schedule";
...
export function ServiceCalendarManager({
  ...
  const selectedOpenWeekdays = normalizeOpenWeekdays(selectedService?.open_weekdays);
  const recurringClosedWeekdays = [1, 2, 3, 4, 5, 6, 7]
    .filter((day) => !selectedOpenWeekdays.includes(day))
    .map((day) => day === 7 ? 0 : day);
        ...
        <div className="grid gap-4 lg:grid-cols-[0.75fr,1.25fr]">
          <div className="space-y-3 rounded-[1rem] border border-border/70 bg-muted/15 p-3">
            ...
            <div className="space-y-2 rounded-[0.95rem] border border-border/70 bg-card px-4 py-3">
              ...
              <p className="text-xs text-muted-foreground">
                Recurring open days: {formatOpenWeekdays(selectedService?.open_weekdays)}
              </p>
            </div>
          ...
          <div className="space-y-3 rounded-[1rem] border border-border/70 bg-muted/10 p-3">
              ...
              <Calendar
                ...
                onDayClick={(day) => {
                  ...
                  if (
                    ...
                    selectedOpenWeekdays.includes(getIsoWeekday(dayKey)) &&
                    (!selectedService?.availability_end_date ||
                ...
                disabled={[
                  ...
                  ...(recurringClosedWeekdays.length ? [{ dayOfWeek: recurringClosedWeekdays }] : []),
            ...
            <p className="px-1 text-xs text-muted-foreground">
              Dates outside the booking window and recurring open days stay disabled automatically. Existing bookings still reduce live availability separately.
            </p>
          </div>
        </div>
